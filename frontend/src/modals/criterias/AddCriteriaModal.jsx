...
export default function AddCriteriaModal({ open, handleClose, updateTableFunction }) {
  ...
  const formik = useFormik({
    ...
    onSubmit: () => {
      ...
      axiosPrivate
        .post('/libraries/addCriteria', formik.values)
        .then(() => {
          enqueueSnackbar('Criteria Added', {
            variant: 'success'
          });
          updateTableFunction();
          formik.resetForm();
  ...
  useEffect(() => {
    const handleClickOutside = event => {
      if (showIconPicker && !event.target.closest('.icon-picker-container')) {
        setShowIconPicker(false);
        setIconSearch('');
      }
    };
